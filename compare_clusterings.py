...
def compute_confusion_matrix(res, file, argmax_stab, argmax_nCl):
    ...
    patno_cluster_b = patno_cluster_b.replace(3,30)
    patno_cluster_b = patno_cluster_b.replace(4,40)
    patno_cluster_b = patno_cluster_b.replace(40,3)
    patno_cluster_b = patno_cluster_b.replace(30,4)
    ...
    plt.figure()
    sns.heatmap(confusion_matrix.T, square = True, annot=True, xticklabels = ["cluster "+str(ii) for ii in ['A', 'B']], yticklabels = ["cluster "+str(ii) for ii in range(1,b+1)], fmt = '.0f' )
...
plt.legend()
plt.savefig('figs/2_vs_5.png', dpi = 300)
...
ax.bar_label(ax.containers[1],fontsize= 12)
plt.legend()
plt.savefig('figs/recluster5to2.png', bbox_inches='tight')
